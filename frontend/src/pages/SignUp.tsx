...
const SignUp = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => { setLoading(false); toast.success("Account Created", "Welcome!"); }, 1000);
  };
      ...
      <div className="w-full max-w-md p-8 rounded-3xl bg-white border border-[var(--color-border)] shadow-xl space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <FormInput label="First Name" name="firstName" value={form.firstName} onChange={handleChange} error={errors.firstName} placeholder="Musa" leftIcon={<User className="w-4 h-4 text-[var(--color-muted)]" />} className="!bg-[var(--color-card-bg)] !border-[var(--color-border)] text-[var(--color-text-dark)]" />
            <FormInput label="Last Name" name="lastName" value={form.lastName} onChange={handleChange} error={errors.lastName} placeholder="Sheikh" leftIcon={<User className="w-4 h-4 text-[var(--color-muted)]" />} className="!bg-[var(--color-card-bg)] !border-[var(--color-border)] text-[var(--color-text-dark)]" />
          </div>
          ...
          <FormInput label="Confirm Password" type="password" name="confirmPassword" value={form.confirmPassword} onChange={handleChange} error={errors.confirmPassword} placeholder="••••••••" leftIcon={<Lock className="w-4 h-4 text-[var(--color-muted)]" />} className="!bg-[var(--color-card-bg)] !border-[var(--color-border)] text-[var(--color-text-dark)]" />
          <Button type="submit" disabled={loading} className="w-full !py-3 text-xs font-bold rounded-xl bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white"><span className="inline-flex items-center justify-center gap-2 whitespace-nowrap">{loading ? "Creating Account..." : "Create Account"}{!loading && <ArrowRight className="w-4 h-4 shrink-0" />}</span></Button>
        </form>
