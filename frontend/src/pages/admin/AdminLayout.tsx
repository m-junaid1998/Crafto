...
const navs = [
  { name: "Dashboard", path: "/admin", icon: LayoutDashboard }, { name: "Orders", path: "/admin/orders", icon: ShoppingBag },
  { name: "Customers", path: "/admin/customers", icon: Users }, { name: "Categories", path: "/admin/categories", icon: FolderTree },
  { name: "Products", path: "/admin/products", icon: Package }, { name: "Media", path: "/admin/media", icon: Image },
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-[var(--color-primary)] text-white flex flex-col justify-between transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div>
          <div className="h-20 flex items-center justify-between px-5 border-b border-white/10">
            <h1 className="font-serif text-sm font-bold tracking-wider uppercase truncate">Home N More <span className="text-[var(--color-accent)] italic font-normal">Studio</span></h1>
            <Button variant="ghost" size="sm" onClick={() => setOpen(false)} className="lg:hidden text-white/60 !p-1"><X className="w-5 h-5" /></Button>
          </div>
          <nav className="p-4 space-y-2">
            {navs.map(({ name, path, icon: Icon }) => (
              <Link key={name} to={path} onClick={() => setOpen(false)} className={`flex items-center gap-3.5 px-4 py-3 rounded-xl text-xs uppercase tracking-widest font-semibold transition-all ${pathname === path ? "bg-[var(--color-accent)] shadow-lg font-bold" : "text-white/70 hover:bg-white/5 hover:text-white"}`}>
    ...
    Logout
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 overflow-hidden">
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-[var(--color-border)] px-4 sm:px-8 flex items-center justify-between shrink-0 z-30">
          <Button variant="ghost" size="sm" onClick={() => setOpen(true)} className="lg:hidden !p-2"><Menu className="w-6 h-6" /></Button>
          ...
          <div className="flex items-center gap-3 ml-auto">
            <Link to="/" target="_blank" className="hidden sm:inline-block"><Button variant="outline" size="sm" leftIcon={<ExternalLink className="w-3.5 h-3.5 text-[var(--color-accent)]" />}>Live Store</Button></Link>
