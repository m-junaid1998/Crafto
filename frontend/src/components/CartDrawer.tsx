    ...
    <>
      ...
      <aside className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-[var(--color-bg-light)] text-[var(--color-text-dark)] z-50 flex flex-col justify-between shadow-2xl p-5 border-l border-[var(--color-border)]">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="flex items-center justify-between pb-3.5 border-b border-[var(--color-border)] shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-[var(--color-muted)] mt-0.5">{totalQty} {totalQty === 1 ? 'item' : 'items'}</p>
              </div>
            </div>
          ...
          {items.length === 0 ? (
            /* EXACT EMPTY STATE UI (MATCHING YOUR IMAGE & THEME) */
            <div className="flex-1 flex flex-col items-center justify-center text-center px-4 py-8 space-y-4">
              ...
              <button
                onClick={onClose}
                className="mt-2 w-full max-w-xs py-3 px-6 rounded-xl border border-[var(--color-border)] hover:border-[var(--color-accent)] hover:bg-[var(--color-card-bg)] text-[var(--color-accent)] font-bold text-xs uppercase tracking-widest transition-all cursor-pointer"
              >
                Continue Shopping
              </button>
            </div>
            ...
            <>
              ...
              <div className="mt-4 p-3 bg-[var(--color-card-bg)] rounded-xl border border-[var(--color-border)] shadow-2xs shrink-0">
                <div className="flex items-center gap-1.5 text-xs font-medium text-[var(--color-text-dark)] mb-2">
                  ...
                  {isFree ? (
                    <span className="text-[var(--color-success)] font-bold">You unlocked FREE Shipping! 🎉</span>
                  ) : (
                    <span>You're <strong className="text-[var(--color-accent)] font-bold">PKR {remaining.toLocaleString()}</strong> away from free shipping</span>
                  )}
                </div>
                <div className="w-full bg-[var(--color-border)] h-1.5 rounded-full overflow-hidden">
                  <div className="bg-[var(--color-accent)] h-full transition-all duration-500 rounded-full" style={{ width: `${Math.min(100, (subtotal / THRESHOLD) * 100)}%` }} />
                </div>
              </div>

              {/* Cart Items List */}
              <div className="mt-4 space-y-2.5 overflow-y-auto no-scrollbar pr-0.5 flex-1">
                {items.map(i => (
                  <div key={i.id} className="flex items-center gap-3 bg-[var(--color-card-bg)] p-2.5 rounded-xl border border-[var(--color-border)] hover:border-[var(--color-accent)] transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        <h3 className="text-xs font-serif font-bold text-[var(--color-text-dark)] truncate">{i.title}</h3>
                        <button onClick={() => removeItem(i.id)} className="text-[var(--color-muted)] hover:text-[var(--color-danger)] cursor-pointer transition-colors">
                          <Trash2 size={13} />
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-1.5">
                        <div className="inline-flex items-center border border-[var(--color-border)] rounded-full bg-[var(--color-bg-light)] px-1.5 py-0.5 shadow-2xs">
                          <button onClick={() => updateQty(i.id, -1)} className="text-[var(--color-muted)] hover:text-[var(--color-primary)] p-0.5 cursor-pointer">
                            <Minus size={10} />
                          </button>
        ...
        {/* Bottom Checkout Actions (Only visible when items exist) */}
        {items.length > 0 && (
          <div className="pt-3 border-t border-[var(--color-border)] space-y-2.5 shrink-0">
            <div className="space-y-1 text-xs text-[var(--color-muted)] bg-[var(--color-card-bg)] p-2.5 rounded-xl border border-[var(--color-border)]">
              ...
              <div className="flex justify-between items-center">
                <span>Delivery</span>
                <span className={isFree ? 'bg-[var(--color-success)]/10 text-[var(--color-success)] font-bold text-[10px] px-2 py-0.5 rounded-full' : 'font-bold text-[var(--color-text-dark)]'}>
                  {isFree ? 'FREE' : `PKR ${SHIPPING}`}
                </span>
              </div>
            </div>
            <div className="flex items-center justify-between px-1">
              <span className="text-xs font-bold uppercase tracking-wider text-[var(--color-muted)]">EST. TOTAL</span>
              <span className="text-lg font-black text-[var(--color-accent)]">PKR {(subtotal + (isFree || subtotal === 0 ? 0 : SHIPPING)).toLocaleString()}</span>
            </div>
            ...
            <button onClick={onClose} className="w-full text-center text-xs font-semibold text-[var(--color-muted)] hover:text-[var(--color-primary)] transition-colors cursor-pointer block">
              Continue Shopping
            </button>
          </div>
        )}
