    ...
    <>
      <nav className="sticky top-0 z-40 bg-[var(--color-bg-light)] text-[var(--color-primary)] px-4 md:px-12 py-3 min-h-[68px] flex items-center shadow-xs">
        <div className="max-w-7xl w-full mx-auto flex items-center justify-between">
          <button onClick={() => setIsOpen(true)} className="md:hidden w-10 h-10 rounded-full bg-[var(--color-primary)] text-white flex items-center justify-center shadow-md active:scale-95" aria-label="Open Menu">
          ...
          <div className="flex items-center space-x-3 md:space-x-5">
            ...
            <button onClick={() => navigate('/wishlist')} className="hidden md:block hover:text-[var(--color-accent)] p-1 transition-colors" aria-label="Wishlist"><Heart size={22} strokeWidth={2} /></button>
      ...
      <button onClick={() => navigate('/wishlist')} className="fixed bottom-5 left-5 z-30 w-11 h-11 rounded-full bg-[var(--color-primary)] text-white flex items-center justify-center shadow-lg active:scale-95 md:hidden" aria-label="Wishlist"><Heart size={22} strokeWidth={2} /></button>
      ...
      <aside className={`fixed top-0 left-0 bottom-0 w-[280px] bg-[var(--color-bg-light)] z-50 transform transition-transform duration-300 ease-in-out md:hidden flex flex-col justify-between shadow-2xl overflow-y-auto no-scrollbar ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div>
          <div className="bg-[var(--color-primary)] text-white px-5 py-4 flex items-center justify-between shadow-sm">
            ...
            <button onClick={() => setIsOpen(false)} className="p-1 hover:bg-white/10 rounded-lg transition-colors" aria-label="Close Menu"><X size={22} strokeWidth={2} /></button>
          </div>
